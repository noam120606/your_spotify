  ...
  return (
    <div {...stylex.props(styles.container, !hasTrack && styles.disabledContainer)}>
      <div {...stylex.props(styles.trackRow)}>
        {coverUrl ? (
          <img src={coverUrl} alt="Track Cover" {...stylex.props(styles.trackCover)} />
        ) : (
        ...
        <div {...stylex.props(styles.trackInfo)}>
          {hasTrack ? <MarqueeText text={title} /> : <Text size="medium">{title}</Text>}
          {artist && (
            <Text
              size="small"
              color="textSecondary"
              xstyle={[styles.truncateText, styles.artistName]}
            >
              {artist}
            </Text>
          )}
