...
export function ArtisticCard({ title, loading, backgroundImageUrl, emphasizedText, detailsText, onClick }: ArtisticCardProps) {
  return (
    <div {...stylex.props(styles.container, onClick && styles.clickable)} onClick={onClick}>
      ...
      {loading ? (
        ...
        <div {...stylex.props(styles.contentContainer)}>
          {emphasizedText && (
            <Text size="xxlarge" weight="bold" style={{ color: '#fff' }} xstyle={styles.emphasizedText}>
              {emphasizedText}
            </Text>
          )}
          {detailsText && (
            <Text size="medium" style={{ color: 'rgba(255,255,255,0.7)' }} xstyle={styles.detailsText}>
              {detailsText}
            </Text>
          )}
        </div>
