...
export const up = async () => {
  ...
  const usersWithSameSpotifyID = allUsers.reduce<Record<string, User[]>>((acc, curr) => {
  ...
  const userWithMultiple = Object.values(usersWithSameSpotifyID).filter((arr) => arr.length > 1);
  ...
  const promises = userWithMultiple.map(async (users) => {
    const firsts = await Promise.all(users.map((u) => getFirstInfo(u._id.toString())));
  ...
  await UserModel.updateMany({}, { $unset: { password: 1, activated: 1 } }, { strict: false });
